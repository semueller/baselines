...
def train(env, policy, rollout_worker, evaluator,
          ...
          save_policies, policy_path, trained_epochs=-1 ,**kwargs):
...
def make_policy_name(envname, idx):
    return ''.join([envname, '_', 'policy', str(idx), '/'])


def get_latest_policy(envs):
    sess = tf.get_default_session()
    saver = tf.train.Saver()
    model_name = 'model.ckpt'
    policy_path_base = './trained/'
    log_base_path = './log/'
    for idx, env in reversed(list(enumerate(envs))):
        policy_name = make_policy_name(env, idx)
        policy_path = ''.join([policy_path_base, policy_name])
        if not os.path.exists(policy_path):
            continue

        epoch_log_path = ''.join([log_base_path, policy_name]) # if policy exists, this should exist as well
        trained_epochs, trained_env = get_progress(epoch_log_path, envname=env)

        saver.restore(sess, policy_path+model_name)
        return trained_epochs, trained_env

    return 0, envs[0]  # default if no policy was found that can be restored


def launch(
    envs, logdir, n_epochs, num_cpu, seed, replay_strategy, policy_save_interval, clip_return, policy_path, with_forces, plot_forces,
    ...
    current_epoch, current_env = get_latest_policy(envs)
...
@click.command()
@click.option('--env', nargs='+',  help='the names of the OpenAI Gym environments that you want to train on')
